...
void tetheredFunc(){
  ...
  //Default value in bluetooth module is 'Ok', dont want 
...
void processData(char data[]){
  if(condition(data, 'l', 'f', 't')) leftTurnTethered();
    ...
    //This function cuts the "lat" and "lng" from the values
    processLatLng(data);
...
void incrementPointers(){
  turnsArrayPointer++;
  latLngArrayPointer += 2;
  if(latLngArray[latLngArrayPointer] == 0.0){
    Serial.print("latLngArrayPointer= ");
    Serial.print(latLngArrayPointer);
    //flash red for destination reached 
    setFlashVariables(255, 0, 0, 5, 0, 500, false, 0, 1);
  }
}
...
void onApproachingTarget(){
  ...
  //Increment te pointers in the turns and latlng arrays
  incrementPointers();
  delay(1000); 
}
...
// Haversine Formula. Stack overflow
// determines the great-circle distance between two points on a sphere given their lng and lat
float calculateLatLngDist(float originLat, float originLng, 
 ...
 // divide the latitude difference by 2. Get the sin squared of it
 // divide the longiude difference by 2. Get the sin squared of it.
 // multiply the cos of lat1 and lat2. Muliply by sin squared of lon difference
 // add the sin squared of lat difference to the result of the above multiplication
 // distance is = 2 by atan2 of the square root of distance 1 and square root of 1 minus distance1
